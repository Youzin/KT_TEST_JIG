...
PORT_STS	port_sts[16] = {
		{ GIN_BAT_DCHG, BAT_DCHG_PIN, 0, 0, 2, 0},
		{ GIN_BAT_CHG, BAT_CHG_PIN, 0, 0, 2, 0},
		{ GIN_BAT_NFB, BAT_NFB_PIN, 0, 0, 2, 0},		
		{ GIN_BAT_PWR, BAT_PWR_PIN, 0, 0, 2, 0},
		{ GIN_AC_FAIL, AC_FAIL_PIN, 0, 0, 2, 0},
		{ GPIO_DI_BAT_SW, DI_BAT_SW, 0, 0, 2, 0},
		//{ GIN_SELHW, SELHW_PIN, 0, 0, 2, 0},
		//{ GIN_SELETH, SELETH_PIN, 0, 0, 2, 0},
		//{ GPIO_BAT_TEST_SW, BAT_TEST_SW, 0, 0, 2, 0},
		{ NULL, },
...
{
	if ( (mode & 0x01) == 0  ) {
		GPIO_WriteBit(GOUT_ETH_SEL, ETH_SEL_PIN, (BitAction) 1);
		printf("ALARM PORT : CONTACT\n");
	}
	else {
		GPIO_WriteBit(GOUT_ETH_SEL, ETH_SEL_PIN, (BitAction) 0);
...
void	relay_on(u16	no)
{
	...
	switch (no) {
		case 1:
			GPIO_SetBits(GOUT_RELAY1, RELAY1_PIN);
			break;
		case 2:
			GPIO_SetBits(GOUT_RELAY2, RELAY2_PIN);
			break;

		case 3:
			GPIO_SetBits(GOUT_RELAY3, RELAY3_PIN);
			break;
		case 4:
			GPIO_SetBits(GOUT_RELAY4, RELAY4_PIN);
			break;
		default:
...
void	relay_off(u16	no)
{
	...
	switch (no) {
		case 1:
			GPIO_ResetBits(GOUT_RELAY1, RELAY1_PIN);
			break;
		case 2:
			GPIO_ResetBits(GOUT_RELAY2, RELAY2_PIN);
			break;

		case 3:
			GPIO_ResetBits(GOUT_RELAY3, RELAY3_PIN);
			break;
		case 4:
			GPIO_ResetBits(GOUT_RELAY4, RELAY4_PIN);
			break;
		default:
			printf("INVALUD RELAY NO= %d\n",no); 
			break;
...
{
	printf("SR ON\n");
	GPIO_WriteBit(GOUT_SR_ON, SR_ON_PIN, (BitAction) 0);
	//GPIO_WriteBit(GOUT_WD, WD_PIN, (BitAction) 0);
}

void sr_off()
{
	printf("SR OFF\n");
	GPIO_WriteBit(GOUT_SR_ON, SR_ON_PIN, (BitAction) 1);
	//GPIO_WriteBit(GOUT_WD, WD_PIN, (BitAction) 1);
...
void WD_reset()
{
	...
	//GPIO_ToggleBits(GOUT_WD, WD_PIN);
	GPIO_WriteBit(GOUT_WD, WD_PIN, (BitAction)(1 - GPIO_ReadOutputDataBit(GOUT_WD, WD_PIN)));
