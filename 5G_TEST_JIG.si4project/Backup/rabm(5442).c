...
#define BMS_PUV		0x0004
#define BMS_CUV		0x0008
#define BMS_HIT		0x0080

int16_t can_tx_count = 0;

uint16_t bms_test_mode = 0;
BMS_STS_T	bms = { 0, 0, 0xaa73, 0x00, 0};
...
uint16_t dac_value[4] = { 3173, 3455, 3131, 3000} ;
...
float	read_bat_V()
{
	...
	return bat_v + rect_sts.ov_offset;	// 180814  offset���� ;
#endif	
...
void check_bat_sts()
{
	...
	alarm_sts.cell_fail = 1;
	#endif

	alarm_sts.bat_disconnect = bms.dis;
}
...
void	rabm_main()
{
	...
	while (1) {
		...
		if ( !(time10ms % 50 ) ) {
			GPIO_WriteBit(GOUT_LED_POWER , LED_POWER_PIN, (BitAction)(1 - GPIO_ReadOutputDataBit(GOUT_LED_POWER , LED_POWER_PIN)));
			//display_status();
			
		}
		
		if ( time10ms % 203 == 1 || bat_can_flag != 0 ) {			
			check_bat_sts();
			//printf("ADC:%d, %d\n", adc_dr[1], adv.batv);
		}
